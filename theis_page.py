...
class TheisPage(PageWindow, QObject):
    ...
    def calculate_theis(self):
        ...
        df = df.loc[
            (TheisPage.start_time <= df["Time"]) & (df["Time"] <= TheisPage.end_time)
        ]
        ...
        if (
            self.adjust_storativity.value() == 0
            and self.adjust_transmissivity.value() == 0
        ):
        ...
        fig.add_trace(
            go.Scatter(x=one_by_u, y=wu, mode="lines+markers", name="Well Function"),
            secondary_y=False,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=t_by_r2_org, y=s_org, mode="lines+markers", name="Actual Data"
            ),
            secondary_y=True,
        )
        ...
        fig.add_trace(
            go.Scatter(x=t_by_r2, y=s, mode="lines+markers", name="Selected Data"),
            secondary_y=True,
        )
        fig.update_xaxes(type="log")
        fig.update_yaxes(type="log")
    ...
    def create_report(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "Save Report",
            f"Theis Report {formatted_datetime}",
            "PDF Files (*.pdf)",
            options=options,
        )
        if file_path:
