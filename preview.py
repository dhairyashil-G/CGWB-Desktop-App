...
class PreviewPage(PageWindow, QObject):
    ...
    def show_plot(self):
        ...
        df_recovery_test_new.rename(
            columns={'Time': 'Time (min)', 'Drawdown': 'Drawdown (m)'}, inplace=True)

        df_pumping_test_new = df_pumping_test.copy()
        df_pumping_test_new.rename(
            columns={'Time': 'Time (min)', 'Drawdown': 'Drawdown (m)'}, inplace=True)
    ...
    def get_well(self, row):
        ...
        print(f'PreviewPage.well_id_global: {PreviewPage.well_id_global}')

    @pyqtSlot(bool)
    def cooper_jacob_analyzed(self, value):
        PreviewPage.is_cooper_jacob_analyzed = True
        print(f'Cooper Jacob Analyzed: {value}')
    ...
    def theis_recovery_analyzed(self, value):
        ...
        print(f'Theis Recovery Analyzed: {value}')
    ...
    def combined_report_save(self):
        if (PreviewPage.is_cooper_jacob_analyzed != True):
            ...
            QMessageBox.warning(
                None, 'Error', 'Please analyze Cooper-Jacob analysis first!')
        elif (PreviewPage.is_theis_analyzed != True):
            self.combined_report_button.setText('Save Combined Report')
            QMessageBox.warning(
                None, 'Error', 'Please analyze Theis analysis first!')
        elif (PreviewPage.is_theis_recovery_analyzed != True):
            self.combined_report_button.setText('Save Combined Report')
            QMessageBox.warning(
                None, 'Error', 'Please analyze Theis Recovery analysis first!')
        else:
            ...
            lst1.append(
                f"Pumping Rate: {well_object.get('PumpingRate')} m³/day")
            lst2.append(
            ...
            lst4 = list()
            lst3.append(
                f"Analysis Start Time: {PreviewPage.cooper_jacob_data_dict.get('start_time')} min")
            lst4.append(
                f"Analysis End Time: {PreviewPage.cooper_jacob_data_dict.get('end_time')} min")
            lst3.append(
                f"x-intercept value : {round(PreviewPage.cooper_jacob_data_dict.get('x_intercept'),3)}")
            lst4.append(
                f"Slope value : {round(PreviewPage.cooper_jacob_data_dict.get('slope'),3)}")
            ...
            reconstructed_fig = pio.from_json(
                PreviewPage.cooper_jacob_data_dict.get('fig_json'))
            ...
            pdf.cell(
                0, 10, f"Transmissivity : {round(PreviewPage.cooper_jacob_data_dict.get('transmissivity'), 3)} m²/day", ln=1)
            pdf.cell(
                0, 10, f"""Storativity : {"{:.8f}".format(PreviewPage.cooper_jacob_data_dict.get('storativity'))}""", ln=1)
            ...
            lst4 = list()
            lst3.append(
                f"Analysis Start Time: {PreviewPage.theis_data_dict.get('start_time')} min")
            lst4.append(
                f"Analysis End Time: {PreviewPage.theis_data_dict.get('end_time')} min")
            for item1, item2 in zip(lst3, lst4):
                pdf.cell(col_width, 10, item1, border=1)
                pdf.cell(col_width, 10, item2, border=1)
                pdf.ln(10)
            ...
            pdf.cell(0, 10, "Graphical Interpretation", ln=1)
            reconstructed_fig = pio.from_json(
                PreviewPage.theis_data_dict.get('fig_json'))
            ...
            pdf.set_font('Arial', 'B', 12)
            pdf.cell(
                0, 10, f"Transmissivity : {round(PreviewPage.theis_data_dict.get('transmissivity'), 3)} m²/day", ln=1)
            pdf.cell(
                0, 10, f"""Storativity : {"{:.8f}".format(PreviewPage.theis_data_dict.get('storativity'))}""", ln=1)
            # pdf.cell(0, 10, f'Root Mean Square Error = {round(mse_error, 3)}%', ln=1)
            pdf.ln(5)
            ...
            lst4 = list()
            lst3.append(
                f"Analysis Start Time: {PreviewPage.theis_recovery_data_dict.get('start_time')} min")
            lst4.append(
                f"Analysis End Time: {PreviewPage.theis_recovery_data_dict.get('end_time')} min")
            lst3.append(
                f"x-intercept value : {round(PreviewPage.theis_recovery_data_dict.get('x_intercept'),3)}")
            lst4.append(
                f"Slope value : {round(PreviewPage.theis_recovery_data_dict.get('slope'),3)}")
            for item1, item2 in zip(lst3, lst4):
                pdf.cell(col_width, 10, item1, border=1)
                pdf.cell(col_width, 10, item2, border=1)
                pdf.ln(10)
            ...
            pdf.cell(0, 10, "Graphical Interpretation", ln=1)
            reconstructed_fig = pio.from_json(
                PreviewPage.theis_recovery_data_dict.get('fig_json'))
            ...
            pdf.set_font('Arial', 'B', 12)
            pdf.cell(
                0, 10, f"Transmissivity : {round(PreviewPage.theis_recovery_data_dict.get('transmissivity'), 3)} m²/day", ln=1)
            pdf.cell(
                0, 10, f"""Delta S : {"{:.3f}".format(PreviewPage.theis_recovery_data_dict.get('deltas'))}""", ln=1)
            # pdf.cell(0, 10, f'Root Mean Square Error = {round(mse_error, 3)}%', ln=1)
            pdf.ln(5)
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self, "Save Report", f"Combined Report {formatted_datetime}", "PDF Files (*.pdf)", options=options)
            if (file_path):
