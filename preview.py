...
class PreviewPage(PageWindow, QObject):
    ...
    def show_plot(self):
        ...
        fig.add_trace(
            go.Scatter(
                x=df_pumping_test["Time"],
                y=df_pumping_test["Drawdown"],
                mode="lines+markers",
                name="Pumping Data",
        ...
        fig.add_trace(
            go.Scatter(
                x=df_recovery_test["Time"],
                y=df_recovery_test["Drawdown"],
                mode="lines+markers",
                name="Recovery Data",
            )
        )
    ...
    def combined_report_save(self):
        if PreviewPage.is_cooper_jacob_analyzed != True:
            ...
            pdf.cell(
                0,
                10,
                f"Transmissivity : {round(PreviewPage.cooper_jacob_data_dict.get('transmissivity'), 3)} m²/day",
                ln=1,
            )
            pdf.cell(
                0,
                10,
                f"""Storativity : {"{:.8f}".format(PreviewPage.cooper_jacob_data_dict.get('storativity'))}""",
                ln=1,
            )
            ...
            pdf.cell(
                0,
                10,
                f"Transmissivity : {round(PreviewPage.theis_data_dict.get('transmissivity'), 3)} m²/day",
                ln=1,
            )
            pdf.cell(
                0,
                10,
                f"""Storativity : {"{:.8f}".format(PreviewPage.theis_data_dict.get('storativity'))}""",
                ln=1,
            )
            # pdf.cell(0, 10, f'Root Mean Square Error = {round(mse_error, 3)}%', ln=1)
            pdf.ln(5)
            ...
            pdf.cell(
                0,
                10,
                f"Transmissivity : {round(PreviewPage.theis_recovery_data_dict.get('transmissivity'), 3)} m²/day",
                ln=1,
            )
            pdf.cell(
                0,
                10,
                f"""Delta S : {"{:.3f}".format(PreviewPage.theis_recovery_data_dict.get('deltas'))}""",
                ln=1,
            )
            # pdf.cell(0, 10, f'Root Mean Square Error = {round(mse_error, 3)}%', ln=1)
            pdf.ln(5)
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self,
                "Save Report",
                f"Combined Report {formatted_datetime}",
                "PDF Files (*.pdf)",
                options=options,
            )
            if file_path:
